...
# ================= 这里修改了第三步，加入了深度清洗和打分机制 =================
PROMPT_STEP_3 = """
你是一位拥有10年实战经验的韩国 Coupang 跨境电商运营专家。整个团队都在中国，除韩文关键词外，所有解释分析必须用纯中文输出。

**核心任务：**
基于产品详情页原图及以下 Naver 关键词真实搜索量数据（CSV格式），输出精准广告分组、否定词表。注意：所有关于关键词的分析绝对不要含有 LxU 的品牌词！
...
第二步：关键词清洗与打分 (Filtering & Scoring) - 极度重要！
基于提供的数据，进行严格的购物意图筛选，严防无转化泛词混入：
1. 相关性打分 (1-5分，1分最高，5分最低)：
   - 1-2分 (保留)：核心词及精准长尾词。
   - 3分 (保留)：强关联场景或竞品词（可用于捡漏）。
   - 4-5分 (严格剔除/否定)：宽泛大词、完全不具备购物搜索属性的词、或材质/属性完全错误的词。
2. 流量与痛点加权：优先保留能解决“竞品痛点”的词。参考“月总搜索量”，保留虽然流量小但极精准的长尾词。
...
用于广告后台屏蔽，防止无效烧钱。
- 建议屏蔽的词：[词1], [词2], [词3]... (列出那些被打为4-5分、不相关、无购物意图的词)
