...
# 0. 页面与 Secrets 配置
# ==========================================
st.set_page_config(page_title="LxU 测品工作流 (SEO+一键复制版)", layout="wide")
...
【💡 极度重要排版要求：一键复制功能】：
为了方便我一键复制，你生成的“纯韩文标题”以及“纯韩文逗号隔开的关键词”，都必须单独放在 Markdown 的代码块（```text 和 ```）里面！
...
# ================= 这里修改了第三步，加入了极其严苛的过滤和数量限制 =================
PROMPT_STEP_3 = """
你是一位拥有10年实战经验的韩国 Coupang 跨境电商高级广告操盘手。整个团队都在中国，除韩文关键词外，所有解释分析必须用纯中文输出。

**核心任务：**
基于产品详情页原图及以下 Naver 关键词海量原始搜索数据（CSV格式），进行【极其严苛的清洗与过滤】，最终只输出能直接用于高 ROAS 投放的精选广告分组和否定词表。注意：所有关于关键词的分析绝对不要含有 LxU 的品牌词！

【市场数据】：
{market_data}

第一步：全维度分析 (视觉属性识别、痛点挖掘、排除逻辑) - 必须纯中文。
请先根据图片深度解析产品的真实材质、核心功能、适用受众和绝对不能适用的场景。

第二步：杀伐果断的清洗与打分 (Filtering & Scoring) - 宁缺毋滥！
你面前是几百上千个杂乱的词，你必须像花自己钱烧广告一样，把不精准的烂词全部杀掉！
1. 严格对比图片：只要词语中含有产品根本没有的属性（如：图片是塑料，词是金属；图片是A功能，词是B功能），直接打 4-5 分并彻底剔除！
2. 剔除无意图泛词：对于太宽泛、只有流量但没有明确“要买东西”意图的资讯词，彻底剔除！
3. 优中选优（极其重要）：【核心出单词】、【精准长尾词】、【捡漏与痛点组】这三个广告组加起来，**总共只允许保留 30 到 50 个最具有转化价值的顶级词汇！** 绝对不允许把几百个词原样吐出来！

第三步：输出二大模块 (Output Modules)
模块一：高价值付费广告投放策略表
【强制格式与排序指令】：
1. 必须严格输出为一个统一的 Markdown 表格！
2. 排序规则（极度重要）：请先按“广告组分类”进行分组排序（展示顺序依次为：核心出单词 ➡️ 精准长尾词 ➡️ 捡漏与痛点组）。
3. 在每一个分类的内部，再严格按照“月总搜索量”从高到低进行降序排列！
4. 保证准确的情况下尽量保证关键词数量。关键词后面不需要带出处的小标志（例如 [1] 等标注）。
5. 必须加入【相关性评分】列，展示你对该词购物意图的打分（保留的词仅限1-3分）。
...
用于广告后台屏蔽，防止无效烧钱。
- 建议屏蔽的词：[列出那些被你打为4-5分、与原图属性冲突、无购物意图的高危词汇，必须至少列出 10-20 个真实的过滤词]
- 屏蔽原因：[纯中文简述，例如：材质不符（我是塑料，词是金属）、场景错误、纯资讯词无购物意图等]
"""

# ==========================================
# 2. Naver 数据抓取函数 (无限制，火力全开)
# ==========================================
...
# 3. 主 UI 与全自动工作流
# ==========================================
st.title("⚡ LxU 自动化测品工厂 (SEO升级 + 复制版)")
