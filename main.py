...
# 0. 页面与 Secrets 配置
# ==========================================
st.set_page_config(page_title="LxU 测品工作流 (分类锁定版)", layout="wide")
...
【市场数据（包含第一步提取的目标词及 Naver 拓展词）】：
{market_data}

第一步：建立“排雷标准”（必须纯中文）
简述该产品的真实材质、核心功能，并明确指出哪些词是绝对不能碰的红线。

第二步：铁血强制分类与输出（极度重要！）
请你基于提供的数据，**优先保留数据中的“原词(目标词)”**，并结合高质量的拓展词，挑选出 40-60 个最具转化价值的词。
...
- 建议屏蔽的词：[用逗号隔开，列出所有触碰红线、无购物意图的垃圾拓展词。必须至少列出 10 个真实的过滤词！]
- 屏蔽原因：[纯中文简述理由]
"""
